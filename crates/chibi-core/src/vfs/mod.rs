...
mod integration_tests {
    ...
    async fn test_vfs_multi_context_integration() {
        ...
        vfs.write(VfsCaller::System, &sys_config, b"{\"version\":1}")
            .await
            .unwrap();
        assert!(vfs.exists(VfsCaller::System, &sys_config).await.unwrap());
        assert_eq!(
            vfs.read(VfsCaller::System, &sys_config).await.unwrap(),
            b"{\"version\":1}"
        );
        ...
        let data = vfs
            .read(VfsCaller::Context("bob"), &alice_notes)
            .await
            .unwrap();
        ...
        let err = vfs
            .write(VfsCaller::Context("bob"), &alice_notes, b"hacked")
            .await
            .unwrap_err();
        assert_eq!(err.kind(), ErrorKind::PermissionDenied);
        ...
        vfs.write(VfsCaller::Context("bob"), &bob_file, b"bob's contribution")
            .await
            .unwrap();

        // bob can write to own home
        let bob_private = VfsPath::new("/home/bob/private.md").unwrap();
        vfs.write(VfsCaller::Context("bob"), &bob_private, b"bob's secret")
            .await
            .unwrap();

        // --- copy/move with permissions ---
        let backup = VfsPath::new("/shared/backup.md").unwrap();
        vfs.copy(VfsCaller::Context("alice"), &shared_tasks, &backup)
            .await
            .unwrap();
        assert_eq!(
            vfs.read(VfsCaller::Context("alice"), &backup)
                .await
                .unwrap(),
            b"# tasks\n- implement vfs"
        );
        ...
        vfs.rename(VfsCaller::Context("bob"), &bob_file, &renamed)
            .await
            .unwrap();
        assert!(
            !vfs.exists(VfsCaller::Context("bob"), &bob_file)
                .await
                .unwrap()
        );
        assert_eq!(
            vfs.read(VfsCaller::Context("bob"), &renamed).await.unwrap(),
        ...
        let err = vfs
            .delete(VfsCaller::Context("bob"), &alice_notes)
            .await
            .unwrap_err();
        assert_eq!(err.kind(), ErrorKind::PermissionDenied);

        // alice deletes her own file
        vfs.delete(VfsCaller::Context("alice"), &alice_notes)
            .await
            .unwrap();
        assert!(
            !vfs.exists(VfsCaller::Context("alice"), &alice_notes)
                .await
                .unwrap()
        );
        ...
        let meta = vfs
            .metadata(VfsCaller::Context("alice"), &backup)
            .await
            .unwrap();
        ...
        let mut home_entries = vfs
            .list(VfsCaller::Context("alice"), &VfsPath::new("/home").unwrap())
            .await
            .unwrap();
        ...
        vfs.mkdir(VfsCaller::Context("alice"), &subdir)
            .await
            .unwrap();
        assert!(
            vfs.exists(VfsCaller::Context("alice"), &subdir)
                .await
                .unwrap()
        );
        let subdir_meta = vfs
            .metadata(VfsCaller::Context("alice"), &subdir)
            .await
            .unwrap();
        ...
        let err = vfs
            .mkdir(
                VfsCaller::Context("alice"),
                &VfsPath::new("/sys/forbidden").unwrap(),
            )
            .await
            .unwrap_err();
        assert_eq!(err.kind(), ErrorKind::PermissionDenied);
